...
def compute_dir_far(Gfeat, Glabel, Pfeat, Plabel, nac_k,matcher="cos", nac_s=1):
...
def save_dir_far_curve(args,Gfeat, Glabel, Pfeat, Plabel, save_dir):
    cos_tensor = compute_dir_far(Gfeat, Glabel, Pfeat, Plabel, nac_k=args.k,matcher='cos')
    nac_tensor = compute_dir_far(Gfeat, Glabel, Pfeat, Plabel, nac_k=args.k,matcher='NAC')
...
def save_dir_far_excel(args,Gfeat, Glabel, Pfeat, Plabel, save_dir):
    cos_tensor = compute_dir_far(Gfeat, Glabel, Pfeat, Plabel, matcher='cos',nac_k=args.k)
    nac_tensor = compute_dir_far(Gfeat, Glabel, Pfeat, Plabel, matcher='NAC',nac_k=args.k)
    cos_list, nac_list = [], []
    ...
    file_name = 'r_'
    if args.base_weight:
        file_name = file_name + 'base_weight_' + str(args.base_weight) + '_'
    df.to_excel(save_dir+'/'+file_name+'.xlsx')
